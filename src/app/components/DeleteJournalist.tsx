...
export default function DeleteJournalistModal({ journalist, isOpen, onClose, onSuccess }: DeleteModalProps) {
  const [processing, setProcessing] = useState(false);
  const { deleteJournalist } = useApi();
  ...
  const handleDelete = async () => {
    ...
    setProcessing(true);
    try {
      await deleteJournalist(journalist.id);
      setProcessing(false);
      onClose();
      if (onSuccess) {
        onSuccess(journalist.id);
      }
        ...
        <div className="flex min-h-full items-center justify-center p-4">
          ...
          >
            <div className="flex justify-between items-center mb-4">
              <p className="text-lg font-medium text-slate-700">Delete Journalist</p>
              <button
                onClick={onClose}
            ...
            <div className="mb-6">
              <div className="mb-4 p-4 bg-red-50 rounded-lg border border-red-100">
                ...
                <p className="text-center font-medium text-red-700">Are you sure you want to delete this journalist?</p>
              </div>
              
              {journalist && (
                <div className="p-4 bg-slate-50 rounded-lg">
                  <h3 className="font-medium text-slate-700 mb-1">{journalist?.name}</h3>
                  <p className="text-sm text-slate-500">{journalist?.email}</p>
                </div>
              )}
              
              <p className="mt-4 text-sm text-slate-600">
                This action cannot be undone. All data associated with this journalist will be permanently removed.
              </p>
            </div>
